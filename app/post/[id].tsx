...
export default function PostDetailScreen() {
  ...
  const handleKakaoMapNavigation = () => {
    ...
    // 카카오맵 스킴: vp1, vp2, ... 최대 5개까지 지원
    const waypointParams = waypoints.slice(0, 5)
      .map((w, idx) => `&vp${idx + 1}=${w.lat},${w.lng}`)
      .join('');

    // 도보 길찾기 URL 스킴
    let url = `kakaomap://route?sp=${start.lat},${start.lng}&ep=${end.lat},${end.lng}&by=FOOT${waypointParams}`;

    console.log('카카오맵 URL:', url); // 실제 URL 확인
    Linking.openURL(url);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonRow}>
        ...
        >
          <Text style={styles.buttonText}>경로 안내 (TMAP)</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
